...
curl -b cookies.txt --cookie-jar cookies.txt -X POST "http://localhost:$port/ultimateFinalization" -H "Content-Type: application/x-www-form-urlencoded" -d "fname=$name&lname=$surname&0=child&1=student&2=adult&3=child&4=student&csrfToken=$csrfToken"